...
class Game():
    ...
    def inicicializa_game(self):
        ...
        self.percebe_empuro_tempo = 80
        self.percebe_empuro_timer = 0
        self.percebe_empuro = False
        ...
        self.inimigo_invisivel = False
        self.inimigo_invisivel_start_frame = 0
        self.inimigo_invisibilidade_tempo = 120
    ...
    def impacto_tiros(self):
        for i in range(len(self.tiros_x)):
            tiro_x = self.tiros_x[i]
            tiro_y = self.tiros_y[i]

            # Verifique se o tiro atingiu o inimigo
            if (tiro_x >= self.x_inimigo and tiro_x <= self.x_inimigo + 30 and
                tiro_y >= self.y_inimigo and tiro_y <= self.y_inimigo + 30):
                # Tiro atingiu o inimigo, ação apropriada aqui
                print("balaço")
                # Por exemplo, você pode remover o tiro e reduzir a vida do inimigo

                self.inimigo_invisivel = True
                self.inimigo_invisivel_start_frame = px.frame_count
                self.x_inimigo = 280
                ...
                del self.tiros_x[i]
                del self.tiros_y[i]
                break  # Saia do loop, pois um tiro só pode atingir um inimigo


    def tapinha(self):
        self.personagem_box = [self.x_inicial, self.y_inicial, self.x_inicial + 30, self.y_inicial + 30]  # Hitbox do jogador
        self.fantasma_box = [self.x_inimigo, self.y_inimigo, self.x_inimigo + 30, self.y_inimigo + 30]  # Hitbox do inimigo

        if (self.personagem_box[0] < self.fantasma_box[2] and
            self.personagem_box[2] > self.fantasma_box[0] and
            self.personagem_box[1] < self.fantasma_box[3] and
            self.personagem_box[3] > self.fantasma_box[1]):

            # Colisão detectada entre jogador e inimigo
            if not self.inimigo_invisivel and not self.percebe_empuro:
                # O jogador e o inimigo não estão invisíveis, lide com a colisão (por exemplo, diminua a vida)
                self.life -= 1
                print("ai")
                self.percebe_empuro = True  # Set the collision flag to True
                self.percebe_empuro_timer = px.frame_count  # Set the timer to the current frame count

        # Reset self.percebe_empuro to False after percebe_empuro_tempo frames
        if self.percebe_empuro and (px.frame_count - self.percebe_empuro_timer >= self.percebe_empuro_tempo):
            self.percebe_empuro = False
    ...
    def update(self):
        ...
        if self.inimigo_invisivel and (px.frame_count - self.inimigo_invisivel_start_frame >= self.inimigo_invisibilidade_tempo):
            self.inimigo_invisivel = False
    ...
    def draw(self):
        ...
        if not self.game_over:
            ...
            px.text(195, 15, "PONTOS = " + str(self.pontos) , px.frame_count % 16, )
            px.text(20, 15, "VIDAS = " + str(self.life) , px.frame_count % 16, )

            px.blt(self.x_inicial, self.y_inicial, 0, 0, 0, 30, 30, 13)

            if not self.inimigo_invisivel:
            ...
            px.text(100, 100, "GAME OVER", px.frame_count % 16)  # Exemplo de mensagem de "Game Over"
            px.text(80, 150, "Pressione Q para sair", px.frame_count % 16)  # Instruções para reiniciar
            px.text(80, 140, "Pressione R para reiniciar", px.frame_count % 16)
