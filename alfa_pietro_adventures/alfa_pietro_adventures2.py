...
class Cronometro:
    ...
    def tempo_decorrido(self):
        ...
        tempo_passado = (tempo_atual - self.tempo_inicial) // 1000  # Tempo em segundos
        return tempo_passado

    def desenhar_cronometro(self, tela):
        tempo_passado = self.tempo_decorrido()
        minutos = tempo_passado // 60
        segundos = tempo_passado % 60
        cronometro_surface = self.fonte.render(f'{minutos:02}:{segundos:02}', True, (0, 0, 153))
        
        # Alinhando o cronômetro com os pontos no eixo Y e colocando no lado esquerdo
        posicao_pontos_x = (self.eixo_x / 4)
        posicao_pontos_y = ((self.eixo_y / 3) - (self.eixo_y // 4.5)) 
        tela.blit(cronometro_surface, (posicao_pontos_x, posicao_pontos_y))  # Exibe o cronômetro no lado esquerdo
...
tempo_imunidade = Tempo()
tempo_imunidade.start(5000)
...
cronometro_instancia = Cronometro(tamanho_eixo_x, tamanho_eixo_y)
...
while running:
    ...
    # Colisões / pontos
    if image_rect.colliderect(cigarro_rect):
    ...
    if tempo_imunidade.has_elapsed():
    ...
    #desenha cronometo
    cronometro_instancia.desenhar_cronometro(screen)
